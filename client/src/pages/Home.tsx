            ...
            <div className="flex flex-col items-center gap-2">
              {result.runId && (
                <div className="flex items-center gap-2">
                  <Badge variant={result.usedCache ? "secondary" : "default"} className="font-mono">
                    {result.usedCache ? "CACHED" : "FRESH RUN"}
                  </Badge>
                  <span className="text-xs text-muted-foreground font-mono">ID: {result.runId}</span>
                </div>
              )}
              {result.timestamp && (
                <div className="flex items-center gap-4 text-sm text-muted-foreground">
                  ...
                  {result.stats?.duration && (
                    <span className="font-mono">{result.stats.duration}s</span>
                  )}
                </div>
              )}
              {result.usedCache && result.cachedAgeSeconds !== undefined && (
                <div className="text-xs text-yellow-600 dark:text-yellow-500">
                  Warning: Showing cached results from {result.cachedAgeSeconds}s ago
                </div>
              )}
            </div>
