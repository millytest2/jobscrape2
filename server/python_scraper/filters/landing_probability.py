...
def score_role_match(job: Dict, target_role: str) -> int:
    ...
    # Exact match (e.g., "Sales Engineer" exactly, not "Pre-Sales Engineer")
    # Use word boundaries to ensure exact match
    import re
    if re.search(r'\b' + re.escape(target_role_lower) + r'\b', title):
        return 100
    
    # Close match (contains all words but not exact, e.g., "Pre-Sales Engineer" for "Sales Engineer")
