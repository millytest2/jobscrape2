...
def main(location: str, role: str, profile_path: str, top_n: int = 20):
    ...
    target_roles = profile.get("target_roles", [role])[:3]  # Limit to top 3 roles for speed
    ...
    # Apply mission-driven filter
    print(f"\n{'='*60}", file=sys.stderr)
    print(f"FILTERING PHASE", file=sys.stderr)
    print(f"{'='*60}", file=sys.stderr)
    print(f"Applying mission-driven filter (threshold >= 35)...", file=sys.stderr)
